...
UNewWeaponComponent::UNewWeaponComponent()
{
...
void UNewWeaponComponent::SetCurve(UCurveFloat* Hor, UCurveFloat* Ver)
{
...
void UNewWeaponComponent::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	if (Character != nullptr)
	{
		// Unregister from the OnUseItem Event
		Character->OnUseItem.RemoveDynamic(this, &UNewWeaponComponent::Fire);
	}
}


void UNewWeaponComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
...
void UNewWeaponComponent::StartHorizontalRecoil(float Value)
{
	if (RecoilTimeline.IsReversing()) { return; }
	Character->AddControllerYawInput(Value);
}

void UNewWeaponComponent::StartVerticalRecoil(float Value)
{
	if (RecoilTimeline.IsReversing()) { return; }
...
void UNewWeaponComponent::StartRecoil()
{
	RecoilTimeline.PlayFromStart();
}

void UNewWeaponComponent::ReverseRecoil()
{
	RecoilTimeline.ReverseFromEnd();
}

void UNewWeaponComponent::AttachWeapon(ARand_RecoilCharacter* TargetCharacter)
{
	...
	if (Character != nullptr)
	{
		...
		Character->OnUseItem.AddDynamic(this, &UNewWeaponComponent::Fire);
